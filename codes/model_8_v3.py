...
# ----------------------------- IMAGE PREPROCESSING (for PNGs) ---------------------------------
def preprocess_image(img_pil):
    """
    Apply contrast-limited adaptive histogram equalization (CLAHE)
    to general PNG medical images (non-fundus).
    """
    ...
    # optional gamma correction for brightness normalization
    gamma = 1.1
    look_up = np.array([((i / 255.0) ** (1 / gamma)) * 255 for i in range(256)]).astype("uint8")
...
class BalancedSplitDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        img = preprocess_image(img)  # PNG image normalization (CLAHE)
        if self.transform:
...
num_classes = 5
input_size = 300  # EfficientNet-B3 recommended
batch_size = 16
num_epochs = 40
patience = 10
...
# ----------------------------- MODEL ---------------------------------
model = EfficientNetBN("efficientnet-b3", pretrained=True, spatial_dims=2, in_channels=3, num_classes=num_classes)
model = model.to(device)

# ---- Layer-wise learning rates ----
optimizer = torch.optim.AdamW([
    {'params': model._conv_stem.parameters(), 'lr': 1e-5},
    {'params': model._blocks.parameters(), 'lr': 5e-5},
    {'params': model._conv_head.parameters(), 'lr': 1e-4},
...
criterion = FocalLoss(alpha=train_dataset.class_weights_tensor.to(device))
