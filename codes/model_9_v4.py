...
config = {
    ...
    # Oversampling
    "use_oversampling": True,    # <-- NEW: enable WeightedRandomSampler
...
# For validation, make sure transforms are val_transform
if use_imagefolder:
...
# -------------------------
# Compute label counts on the training subset
...
print("Class counts (train):", label_counts)

# Class frequencies
freq = label_counts / float(label_counts.sum() + 1e-8)

# Class weights for loss (inverse freq, normalized)
weights_for_loss = 1.0 / (freq + 1e-6)
weights_for_loss = weights_for_loss / weights_for_loss.mean()

class_weights = None
if config["use_class_weights"]:
    class_weights = torch.tensor(weights_for_loss, dtype=torch.float32).to(device)
    print("Class weights (for loss):", weights_for_loss)

# Sample weights for oversampling (WeightedRandomSampler)
if config["use_oversampling"]:
    print("🎯 Using WeightedRandomSampler for oversampling minority classes.")
    # Per-class sampling weights = inverse class frequency
    inv_freq = 1.0 / (freq + 1e-8)
    inv_freq = inv_freq / inv_freq.mean()  # normalize (not strictly required)
    sample_weights = np.array([inv_freq[lab] for lab in train_labels], dtype=np.float64)
...
# -------------------------
# Student Setup (with persistence images)
# -------------------------
...
# student - efficient_net_b2
# teacher - efficient_net_b5
# tda -> cbam
