...
config = {
    ...
    "backbone": "retfound",  # Use RetFound model backbone
    "num_classes": 5,
    "batch_size": 8,
    "epochs": 40,
    "lr": 1e-4,
    ...
    "gradcam_out_dir": "RetFound_Model_GradCam",
    "image_size": 224,
...
class RetFoundWithCBAM_PI(nn.Module):
    """
    RetFound backbone + optional CBAM + persistence-image TDA branch with MLP head.
    """
    def __init__(self, backbone_name='retfound', num_classes=5, use_cbam=True,
                 pretrained=True, use_tda=False, tda_img_size=32, tda_mlp_dims=[512, 128]):
        super().__init__()

        # Use RetFound model as the feature extractor
...
# Loss & Trainer
# -------------------------
# Continue from the trainer and loss setup as in your original code.

# Execution flow continues below...
...
def generate_gradcam(model, img_tensor, class_idx, target_layer):
    ...
    def forward_hook(module, inp, out):
        # For CBAM, the output shape is [B, C, H, W]
        # Just store the output tensor directly
