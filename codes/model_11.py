...
# --- ADDED: QWK and MAE imports ---
from sklearn.metrics import classification_report, confusion_matrix, cohen_kappa_score, mean_absolute_error
import timm
...
config = {
    ...
    # --- MODIFIED: Stability Settings ---
    "lr": 5e-5,                  # Slightly reduced from 1e-4
...
# --- Ordinal Loss Class (Squared EMD) ---
class OrdinalEMDLoss(nn.Module):
    """
    Ordinal Regression using Squared Earth Mover's Distance.
    Operates on standard logits (5 classes), keeping architecture same.
    """
    ...
    def forward(self, logits, targets):
        ...
        # 2. Compute CDF (Cumulative Distribution Function) of predictions
        pred_cdf = torch.cumsum(probs, dim=1)
        
        # 3. Create Target CDF
        # If target is 2: CDF should be [0, 0, 1, 1, 1]
        levels = torch.arange(self.num_classes, device=logits.device).unsqueeze(0)
        # Target CDF is 1 where level >= target, else 0
        target_cdf = (levels >= targets.unsqueeze(1)).float()
        
        # 4. MSE between CDFs
        loss = torch.mean(torch.sum((pred_cdf - target_cdf) ** 2, dim=1))
        return loss
...
# 1. Setup Base Loss (MODIFIED FOR ORDINAL)
# We use OrdinalEMDLoss instead of CrossEntropy
print("ℹ Using Ordinal EMD Loss for Base Loss.")
base_loss = OrdinalEMDLoss(num_classes=config["num_classes"]).to(device)
        ...
        for ep in range(config["teacher_epochs"]):
            ...
            with torch.no_grad():
                for imgs, labels in val_loader:
                    ...
                    # Note: Using EMD loss for validation tracking too
...
# -------------------------
# 6. Evaluation (Modified for Ordinal Metrics & Smooth Plots)
# -------------------------
...
plt.plot(smooth_curve(train_losses), label='Smoothed')
...
print("\n🔥 Generating Grad-CAMs (5 per class)...")

target_layer = get_gradcam_target_layer(model)

# --- MODIFIED: Track count per class instead of boolean ---
# Target: 5 images per class
IMAGES_PER_CLASS = 5
cls_counts = {i: 0 for i in range(config["num_classes"])}
...
for ds_idx in indices:
    # Check if we have gathered 5 images for all classes to stop early
    if all(count >= IMAGES_PER_CLASS for count in cls_counts.values()):
        break

    img, label = dataset_ref[ds_idx]

    # Skip if we already have 5 images for this class
    if cls_counts[label] >= IMAGES_PER_CLASS:
        continue
    ...
    # Save with index tracking (e.g., class_Mild_1.jpg, class_Mild_2.jpg)
    current_count = cls_counts[label] + 1
    save_path = os.path.join(
        config["gradcam_out_dir"],
        f"gradcam_class_{class_names[label]}_{current_count}.jpg"
    )
    
    cv2.imwrite(save_path, overlay)
    print(f"✔ Saved Grad-CAM for class {class_names[label]} ({current_count}/{IMAGES_PER_CLASS}) → {save_path}")
