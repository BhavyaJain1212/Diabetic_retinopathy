...
# (Green Channel + CLAHE + Color Amplification)
# -------------------------
class DRPreprocess:
    def __call__(self, img):
        ...
        # We replicate it to 3 channels so the model (pre-trained on RGB) accepts it
        if len(img_np.shape) == 3:
            g = img_np[:, :, 1]
        else:
            g = img_np # Handle already grayscale
            
        img_np = np.stack([g, g, g], axis=-1)

        # 2. CLAHE (Contrast Limited Adaptive Histogram Equalization)
        clahe = cv2.createCLAHE(clipLimit=2.0, tileGridSize=(8, 8))
        # Apply to all 3 channels (which are identical now)
        img_np[:,:,0] = clahe.apply(img_np[:,:,0])
        img_np[:,:,1] = clahe.apply(img_np[:,:,1])
        img_np[:,:,2] = clahe.apply(img_np[:,:,2])

        # 3. Pixel Color Amplification (Ben Graham's Method)
        # Formula: image = image * 4 - gaussian_blur(image) * 4 + 128
...
train_transform = transforms.Compose([
    ...
    DRPreprocess(), # <--- Custom Preprocessing Added
...
class RetinopathyDataset(Dataset):
    ...
    def get_labels(self):
        # Helper to extract all labels for stratification
        labels = []
        for idx in range(len(self.data)):
            ...
            if isinstance(label_val, (int, float)) or str(label_val).isdigit():
                ...
                labels.append(self.folder_names.index(folder_name))
        return labels
...
# Apply transforms specifically
if use_imagefolder:
    # Need to create copies to apply different transforms
    # But since ImageFolder applies transform at init, we'll handle this via Subset wrapping
    # Ideally, we set the transform dynamically, but Dataset/ImageFolder doesn't support easy per-idx transform switching.
    # We will rely on the dataset loaded with train_transform for training.
    # For val, we ideally reload or wrap. For simplicity here, we assume val uses the same resize/norm.
    # To do it correctly with the 'val_transform' defined above:
    val_dataset_base = datasets.ImageFolder(root=config["img_root"], transform=val_transform)
    train_ds = Subset(dataset, train_indices)
    val_ds = Subset(val_dataset_base, val_indices)
else:
    # CSV Dataset handles transform in __getitem__, but we initialized with train_transform
    # Create a separate instance for val
    val_dataset_base = RetinopathyDataset(config["csv_path"], config["img_root"], transform=val_transform)
    train_ds = Subset(dataset, train_indices)
    val_ds = Subset(val_dataset_base, val_indices)

# --- Balancing: Undersampling Majority / Oversampling Minority ---
# We use WeightedRandomSampler on the TRAINING set only.
...
train_loader = DataLoader(
    ...
    sampler=sampler, # <--- Used for balancing
    num_workers=4, 
    pin_memory=True
)
...
val_loader = DataLoader(
    val_ds, 
    batch_size=config["batch_size"],
    shuffle=False, # Validation must not be shuffled for consistency
    num_workers=2, 
    pin_memory=True
)

print(f"Dataset: {len(dataset)} images → train: {len(train_ds)}, val: {len(val_ds)}")
print("✅ Applied Stratified Splitting.")
print("✅ Applied WeightedRandomSampler (Oversampling Minority/Undersampling Majority).")
...
def extract_persistence_image_batch(images_tensor, tda_img_size=32):
    ...
    # Use skew=True because diagrams are in (birth, death) coordinates.
    global pimgr
    if pimgr is not None and (getattr(pimgr, "fitted_", False) is False):
        ...
        if len(non_empty_diags) > 0:
            ...
            # Alternatively, you can leave as is and just resize with cv2 below.
        else:
    ...
    for dgm in diagrams:
        if dgm.shape[0] == 0:
            ...
            pim = pimgr.transform(dgm, skew=True)  # returns 2D image
...
class EfficientNetWithCBAM_PI(nn.Module):
    """
    EfficientNet backbone + optional CBAM + persistence-image TDA branch with MLP head.
    """
...
# Note: Since we are using WeightedRandomSampler, the batches are already balanced.
# Therefore, using class weights in the Loss function as well might over-correct.
# However, for Distillation and robustness, we can keep them or slighty dampen them.
# The previous logic calculated weights on the FULL dataset. We should use TRAIN labels now.

class_weights = None
if config["use_class_weights"]:
    # Calculate weights based on TRAINING split only
    train_label_counts = np.bincount(train_labels)
    freq = train_label_counts / float(train_label_counts.sum())
    weights = 1.0 / (freq + 1e-6)
    weights = weights / weights.mean()
    class_weights = torch.tensor(weights, dtype=torch.float32).to(device)
    print("Class weights (Loss):", weights)
...
for ds_idx in indices:
    ...
    # We need the numeric label to check seen classes
